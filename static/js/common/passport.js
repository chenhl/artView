        ...
        function (window) {
            function SohuPassport(options) {
                this.options = utils.extend({},
                        ...
                        this.options.gidinf && this.options.serialno && (setCookie("gidinf", this.options.gidinf), setCookie("srn", this.options.serialno)),
                        setCookie("t", nonce),
            ...
            var browser = function () {
                ...
                return {
                    ...
                    prefix = browser.ie && browser.ie < 9 ? "https://v4.passport.sohu.com/i/".replace("https", "http") : "https://v4.passport.sohu.com/i/",
                    defaultOptions = ("https://msapi.t.sohu.com", "https://m.passport.sohu.com", {
                        appid: "",
                        url: {
                            ...
                            getPhoneCode: prefix + "mobile/phonecode",
                            getMobileCaptcha: prefix + "smcode/mobile/v2",
                            verifyMobileCaptcha: prefix + "verify/smcode/mobile",
                            getSecmobileCaptcha: prefix + "smcode/secmobile",
                            verifySecmobileCaptcha: prefix + "verify/smcode/secmobile",
                            queryCertificateStaus: prefix + "certificate/query/passport",
                            userCertificate: prefix + "certificate/create"
                        }
                    }),
                    utils = {
                        extend: function () {
                            var i, name, options, copy, target = arguments[0] || {},
            ...
            return SohuPassport.prototype = {
                ...
                createIFrame: function (name) {
                    ...
                    var ifa;
                    return browser.ie && browser.ie < 9 ? ifa = document.createElement('<iframe src="about:blank" name="' + name + '"></iframe>') : (ifa = document.createElement("iframe"), ifa.setAttribute("name", name)),
                ...
                getJSONP: function (url, callback, charset) {
                    var ret, cbName = this.randomName("cb"),
                ...
                getPhoneCode: function (args) {
                    this.getMethod(args, this.options.url.getPhoneCode)
                },
                ...
                getMobileCaptcha: function (args) {
                    args.params.pagetoken = pageTokenPhone;
                    var url = this.options.url.getMobileCaptcha + "/" + args.params.captchaType;
                    this.getMethod(args, url)
                },
                verifyMobileCaptcha: function (args) {
                    var url = this.options.url.verifyMobileCaptcha;
                    this.getMethod(args, url)
                },
                getSecmobileCaptcha: function (args) {
                    var url = this.options.url.getSecmobileCaptcha + "/" + args.params.captchaType;
                    this.getMethod(args, url)
                },
                verifySecmobileCaptcha: function (args) {
                    var url = this.options.url.verifySecmobileCaptcha + "/" + args.params.captchaType;
                    this.getMethod(args, url)
                },
                queryCertificateStaus: function (args) {
                    this.getMethod(args, this.options.url.queryCertificateStaus)
                },
                userCertificate: function (args) {
                    this.getMethod(args, this.options.url.userCertificate)
                }
            },
